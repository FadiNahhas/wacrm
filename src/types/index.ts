...
export interface Contact {
  ...
  /** Operator-owned display name. Only ever written by a human (contact
   *  form, CSV import) or an explicit API call — never by the inbound
   *  webhook. Null until someone names the contact. */
  name?: string | null;
  /** The contact's own WhatsApp profile name ("pushname") as reported by
   *  Meta on inbound messages. Contact-controlled and refreshed on every
   *  message, so it must never be written to `name`. Render contacts via
   *  `contactDisplayName` (@/lib/contacts/display-name) rather than
